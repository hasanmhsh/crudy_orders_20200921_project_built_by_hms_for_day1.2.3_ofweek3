...
public class OrderServiceImpl implements OrderService {
    ...
    public Order createNewIfIdIsZeroElseReplaceExisting(Order order) throws EntityNotFoundException, InvalidPropertyException {
        Order newOrder = null;
        Customer cust = null;
        if (order.getCustomer() != null && order.getCustomer().getCustcode() != 0) {
            cust = customerRepository.findById(order.getCustomer().getCustcode())
    ...
    @Transactional
    @Override
    public Order updateExisting(Order order, long id) {
        return null;
    }
